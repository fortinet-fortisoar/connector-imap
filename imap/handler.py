...
def validate_listener_port(listener_port):
    # Escape the listener port using shlex.quote to prevent shell injection attacks
    escaped_port = shlex.quote(str(listener_port))
    # Use a list for the lsof command
    lsof_cmd = ['/usr/sbin/lsof', '-titcp:' + escaped_port]
    pid_output = subprocess.check_output(lsof_cmd).decode("utf-8").strip()
    if not pid_output:  # Check if pid is empty
        raise subprocess.CalledProcessError(returncode=-1, cmd='', output='No process is using the port')
    # Use a list for the ps command
    ps_cmd = ['ps', '-f', '-o', 'cmd', '-p', pid_output]
    cmd = subprocess.check_output(ps_cmd).decode("utf-8")
    cmd_list = cmd.split()
...
class ConfigHandler:
    def __init__(self, config={}):
        ...
        self._config['password'] = base64.b64encode(config.get('password','').encode('utf-8')).decode('utf-8')
